...
impl Existing {
    ...
    pub(crate) fn refusal_reason(&self, path: &std::path::Path) -> String {
        match self {
            ...
            Existing::Absent | Existing::Text(_) => {
                format!("{} is not ours (no marker)", path.display())
            }
        }
    }
}
...
pub(crate) fn remove_backup_if_ours(
    path: &std::path::Path,
    is_ours: fn(&str) -> bool,
...
mod tests {
    ...
    fn unreadable() -> Existing {
        Existing::Unreadable(std::io::Error::new(
    ...
    fn plan_install_refuses_foreign_plugin_without_force() {
        ...
        assert_eq!(
            plan_install(&foreign, OPENCODE_PLUGIN_JS, true, opencode_plugin_is_ours),
            InstallPlan::Write
        );
    }
    ...
    fn remove_backup_if_ours_keeps_a_foreign_backup() {
        ...
        assert_eq!(
            remove_backup_if_ours(&path, opencode_plugin_is_ours),
            None,
    ...
    fn refusal_reason_names_unreadable_and_foreign_files() {
        ...
        assert_eq!(
            Existing::Text("x".into()).refusal_reason(p),
            "/x/zj-radar.js is not ours (no marker)"
        );
        assert!(unreadable()
            .refusal_reason(p)
            .starts_with("/x/zj-radar.js could not be read ("));
    }

    #[test]
    fn plan_uninstall_distinguishes_absent_from_foreign() {
        assert_eq!(
            plan_uninstall(&Existing::Absent, opencode_plugin_is_ours),
            UninstallPlan::Absent
        );
        assert_eq!(
            plan_uninstall(
                &Existing::Text("// not ours\n".to_string()),
                opencode_plugin_is_ours
            ),
            UninstallPlan::NotOurs
        );
        assert_eq!(
            plan_uninstall(&unreadable(), opencode_plugin_is_ours),
            UninstallPlan::NotOurs
        );
        assert_eq!(
            plan_uninstall(
