...
/// Two panes in tab 0 with `focused` holding focus — the manifest shape the
/// fast-path tests re-report with only the focus flag moved.
fn two_pane_manifest(focused: u32) -> PaneUpdate {
    let with_focus = |id: u32| TerminalPane { focused_in_tab: id == focused, ..pane(id) };
    pane_update(HashMap::from([(0, vec![with_focus(7), with_focus(8)])]))
}

#[test]
fn identical_manifest_is_a_no_op_that_keeps_the_rows_memo() {
    // Zellij re-reports the manifest on focus moves, resizes, mouse state,
    // and bells. One that changes no topology must not invalidate the memo
    // (a generation bump is a full re-derive on the next rows() read),
    // request a repaint, or run the store/prune machinery.
    let mut radar = RadarState::default();
    radar.tabs_changed(vec![tab(10, 0, "work", true)]);
    radar.status_mut().apply(payload_in_repo(7, Status::Running, "repo"), 1, 0);
    radar.panes_changed(two_pane_manifest(7), 1, 0, config::NamingMode::Off);
    let generation = radar.generation();

    let change = radar.panes_changed(two_pane_manifest(7), 2, 0, config::NamingMode::Off);

    assert_eq!(radar.generation(), generation, "identical manifest must not bump the generation");
    assert!(!change.render, "nothing to repaint");
    assert_eq!(change.snapshot, SnapshotWrite::None);
    assert!(change.renames.is_empty());
    assert!(change.settle, "focus is still trustworthy on a manifest, so the notifier may settle");
    assert_eq!(radar.status_store().get(7).map(|o| o.status), Some(Status::Running), "stores untouched");
}

#[test]
fn focus_only_manifest_moves_focus_without_touching_rows() {
    let mut radar = RadarState::default();
    radar.tabs_changed(vec![tab(10, 0, "work", true)]);
    radar.panes_changed(two_pane_manifest(7), 1, 0, config::NamingMode::Off);
    assert_eq!(radar.last_focused(), Some(7));
    let generation = radar.generation();

    let change = radar.panes_changed(two_pane_manifest(8), 2, 0, config::NamingMode::Off);

    assert_eq!(radar.last_focused(), Some(8), "the notifier's suppression target follows focus");
    assert_eq!(radar.generation(), generation, "focus is not rows-visible");
    assert!(!change.render);
}

#[test]
fn focus_only_manifest_still_renames_by_the_focused_pane_repo() {
    // The focused pane's repo is a naming candidate, so a focus move inside
    // the tab is the one thing the fast path must still hand to the namer.
    let mut radar = RadarState::default();
    radar.tabs_changed(vec![tab(10, 0, "Tab #1", true)]);
    radar.own_plugin_tab_changed(Some(0));
    radar.status_mut().apply(payload_in_repo(7, Status::Running, "alpha"), 1, 0);
    radar.status_mut().apply(payload_in_repo(8, Status::Running, "beta"), 1, 0);
    let first = radar.panes_changed(two_pane_manifest(7), 1, 0, config::NamingMode::Managed);
    assert_eq!(first.renames.iter().map(|r| r.name.as_str()).collect::<Vec<_>>(), vec!["alpha"]);

    let moved = radar.panes_changed(two_pane_manifest(8), 2, 0, config::NamingMode::Managed);
    assert_eq!(
        moved.renames.iter().map(|r| r.name.as_str()).collect::<Vec<_>>(),
        vec!["beta"],
        "focus moved to the beta pane, so the tab follows"
    );
}
...
fn a_title_change_or_new_pane_takes_the_full_path() {
    ...
    let retitled = pane_update(HashMap::from([(0, vec![
        TerminalPane { title: "vim".into(), ..pane(7) },
        pane(8),
    ])]));
    radar.panes_changed(retitled, 2, 0, config::NamingMode::Off);
    assert!(radar.generation() > generation, "a title change is rows-visible");

    let generation = radar.generation();
    let grown = pane_update(HashMap::from([(0, vec![pane(7), pane(8), pane(9)])]));
    let change = radar.panes_changed(grown, 3, 0, config::NamingMode::Off);
    assert!(radar.generation() > generation, "a new pane changes the live set");
    assert!(change.render);
}
