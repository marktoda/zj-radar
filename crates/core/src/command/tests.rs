...
fn classify_display_keeps_useful_subcommands_and_drops_flags() {
    assert_eq!(
        display(&argv(&["cargo", "test", "render::tests", "--features", "cli", "--", "--nocapture"])),
        "cargo test render::tests"
    );
    assert_eq!(display(&argv(&["codex", "--dangerously-bypass-approvals-and-sandbox", "--model", "gpt-5"])), "codex");
    assert_eq!(display(&argv(&["npm", "run", "build", "--", "--watch"])), "npm run build");
    assert_eq!(
...
fn display_remote_parses_ssh_argv_shapes() {
    ...
    assert_eq!(display(&argv(&["ssh", "ssh://user@prod-db:2222/"])), "ssh prod-db");
    // A `-N -L` tunnel has no destination-following command — still Remote.
    assert_eq!(classify(&argv(&["ssh", "-N", "-L", "8080:localhost:80", "prod-db"])).1, Kind::Remote);
    ...
    assert_eq!(display(&argv(&["mosh", "--predict", "adaptive", "prod-db"])), "mosh prod-db");
...
fn classify_routes_remote_launchers_by_trailing_command() {
    ...
    assert_eq!(classify(&argv(&["ssh", "box", "cargo", "build"])).1, Kind::Command);
...
fn wrappers_and_env_prefixes_are_peeled_before_classification() {
    ...
    for (args, want_msg, want_kind) in cases {
        ...
        let s = store.get(1).unwrap_or_else(|| panic!("{args:?} should be tracked"));
...
fn direnv_export_hook_is_ignored_like_a_prompt() {
    ...
    assert!(!store.pending.contains_key(&1), "direnv export must not open a new command lifecycle");
...
fn rerun_with_same_exit_code_is_not_swallowed_by_dedup() {
    ...
    assert!(!store.needs_ticks(), "a finished re-run must not keep the timer armed");
}
...
fn absolute_argv0_path_basename_used_for_command_and_repo() {
    ...
    let cmd = vec!["/nix/store/abc123-cargo-1.0/bin/cargo".to_string(), "build".to_string()];
...
fn needs_ticks_reflects_state() {
    ...
    let leave_tick = promote_tick + 1;
    store.on_command_changed(1, &[], false, None, leave_tick);
    assert!(store.needs_ticks(), "still active until the debounce window flips it to Done");

    // Timer past debounce → Done (no pending, no Running).
    store.on_timer(Tick(leave_tick + DEBOUNCE_TICKS), EpochSecs(0));
    assert!(!store.needs_ticks(), "false once Done (no pending, no Running)");
}
...
fn ignore_set_covers_all_shells() {
    ...
    for shell in &["zsh", "bash", "fish", "sh", "dash", "starship"] {
        ...
        store.on_command_changed(1, &cmd, true, None, 1);
        assert!(!store.pending.contains_key(&1), "{} must not enter pending", shell);
...
fn agent_foreground_commands_are_not_tracked() {
    ...
    for agent in &["claude", "codex", "opencode", "pi"] {
        ...
        assert!(!store.pending.contains_key(&1), "{agent} must not enter pending");
        store.on_timer(Tick(2), EpochSecs(0));
        assert!(store.get(1).is_none(), "{agent} must leave no resolved command state");
    }
}
...
fn gemini_foreground_command_is_tracked() {
    ...
    store.on_timer(Tick(1 + DEBOUNCE_TICKS), EpochSecs(0));
    let s = store.get(1).expect("gemini must leave a resolved command observation");
    assert_eq!(s.status, Status::Running);
...
fn leaving_foreground_debounces_before_marking_done() {
    ...
    let leave_tick = promote_tick + 1;
    store.on_command_changed(1, &[], false, None, leave_tick);
    assert_eq!(store.get(1).unwrap().status, Status::Running, "leaving the foreground must not flip to Done instantly");
...
fn empty_foreground_command_is_never_promoted() {
    ...
    assert!(!store.pending.contains_key(&1), "empty fg argv must not enter pending");
    store.on_timer(Tick(2), EpochSecs(0));
    assert!(store.get(1).is_none(), "empty fg command must leave no resolved state (no blank Running row)");
}
...
fn different_exit_code_on_displayed_completion_still_displaces() {
    ...
    let receded = s.on_exit(9, Some(2), Tick(50), EpochSecs(999)).expect("the displayed Done leaves via displacement");
...
fn interactive_set_disjoint_from_prompt_and_agent_names() {
    ...
    for name in DEFAULT_INTERACTIVE {
        ...
        assert!(!DEFAULT_REMOTE.contains(name), "{name} must not double as a remote-session launcher");
    }
...
fn display_first_token_is_the_exe_basename() {
    ...
    for case in cases {
        ...
        assert_eq!(first.trim_start_matches('-'), name, "display {display:?} must lead with the exe for {case:?}");
    }
}
...
fn interactive_command_never_promotes_and_never_arms_the_timer() {
    ...
    assert!(!s.needs_ticks(), "a quiet pending must not pin the 1 Hz cadence");
...
fn set_interactive_extras_sweeps_promoted_rows_level_triggered() {
    ...
    assert_eq!(s.quiet_identity(1), Some(("gdb", Kind::Command)), "demote reconstructs the quiet pending");
    ...
    assert_eq!(s.get(1).unwrap().status, Status::Running, "swept row re-promotes after the extra is removed");
    let changed = s.set_interactive_extras(["gdb"]);
...
fn set_remote_extras_is_a_no_op_for_default_remote_names() {
    ...
    assert_eq!(s.get(1).unwrap().kind, Kind::Command, "ssh is a DEFAULT_REMOTE name — the sweep must not touch it");
}
...
fn display_remote_keeps_scanning_options_after_the_destination() {
    ...
    assert_eq!(classify(&argv(&["mosh", "prod-db", "--predict", "adaptive"])), ("mosh prod-db".into(), Kind::Remote));
...
fn remote_destination_keeps_ipv6_literals_whole() {
    ...
    assert_eq!(classify(&argv(&["mosh-client", "::1", "60001"])), ("mosh-client ::1".into(), Kind::Remote));
...
fn service_done_confirm_still_arms_the_timer() {
    ...
    s.on_command_changed(1, &argv(&["zsh"]), true, None, DEBOUNCE_TICKS + 1);
    assert!(s.needs_ticks(), "the armed Done confirm is a scheduled one-shot — it needs ticks");
