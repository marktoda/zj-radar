...
fn is_shell_prompt_covers_non_posix_shells_and_login_argv0() {
    ...
    for shell in [
        "nu", "nushell", "pwsh", "tcsh", "csh", "ksh", "mksh", "ash", "elvish", "xonsh",
    ] {
...
fn display_remote_parses_ssh_argv_shapes() {
    ...
    assert_eq!(
        display(&argv(&["ssh", "-p", "2222", "prod-db"])),
        "ssh prod-db"
    );
    ...
    assert_eq!(
        display(&argv(&["ssh", "-oProxyJump=x", "prod-db"])),
        "ssh prod-db"
    );
    ...
    assert_eq!(
        display(&argv(&["ssh", "-J", "jump", "prod-db"])),
        "ssh prod-db"
    );
    ...
    assert_eq!(
        display(&argv(&["mosh-client", "1.2.3.4", "60001"])),
        "mosh-client 1.2.3.4"
    );
    assert_eq!(
        classify(&argv(&["mosh-client", "1.2.3.4", "60001"])).1,
        Kind::Remote
    );
}
...
fn classify_routes_remote_launchers_by_trailing_command() {
    ...
    assert_eq!(display(&argv(&["ssh", "box", "cargo", "build"])), "ssh box");
}
...
fn classify_display_covers_every_tool_rule_path() {
    let cases: &[(&[&str], &str)] = &[
        ...
        (
            &["python3.12", "-m", "pytest", "-q", "t.py"],
            "python3.12 -m pytest t.py",
        ),
...
fn kind_classification_cases() -> Vec<(Vec<String>, &'static str, Kind)> {
    ...
    vec![
        ...
        (
            argv(&["codex", "--dangerously-bypass-sandbox"]),
            "codex",
            Kind::Codex,
        ),
        ...
        (
            argv(&["cargo", "run", "test-server"]),
            "cargo run test-server",
            Kind::Command,
        ),
        (
...
fn wrappers_and_env_prefixes_are_peeled_before_classification() {
    ...
    let cases: &[(&[&str], &str, Kind)] = &[
        (
            &["RUST_LOG=debug", "cargo", "test", "render"],
            "cargo test render",
            Kind::Test,
        ),
...
fn classify_source_round_trips_through_kind() {
    ...
    for (cmd, display, _) in kind_classification_cases() {
        ...
        assert_ne!(
            kind,
            Kind::Other,
            "{display:?} classified as the Other sentinel"
        );
        assert_eq!(
...
fn fg_command_stays_pending_until_debounce() {
    ...
    if DEBOUNCE_TICKS > 1 {
        ...
        assert!(
            store.get(1).is_none(),
            "still pending one tick short of debounce"
        );
    }
...
fn on_timer_reports_observation_changes_for_snapshot_persistence() {
    ...
    assert!(
        !store.on_timer(Tick(1), EpochSecs(0)).changed,
        "empty store: quiet tick"
    );
    ...
    assert_eq!(
        store
            .on_timer(Tick(promote_tick), EpochSecs(0))
            .changed_panes,
        [1],
        "debounced promotion names its pane"
    );
    assert!(
        !store.on_timer(Tick(promote_tick + 1), EpochSecs(0)).changed,
        "already Running: quiet tick"
    );
    ...
    assert_eq!(
        store.on_timer(Tick(done_tick), EpochSecs(0)).changed_panes,
        [1],
        "confirmed Done-flip names its pane"
    );
    assert!(
        !store.on_timer(Tick(done_tick + 1), EpochSecs(0)).changed,
        "terminal Done: quiet tick"
    );
}
...
fn direnv_export_hook_is_ignored_like_a_prompt() {
    ...
    assert_eq!(
        s.msg, "cargo test",
        "the finished command is cargo test, not direnv"
    );
}
...
fn done_recedes_to_idle_after_ttl_and_reports_the_recede() {
    ...
    assert!(
        ...
        "idle row stays a muted row, not removed"
    );
    assert!(after.changed);
    assert_eq!(after.receded.len(), 1);
    assert_eq!(
        after.receded[0].1.status,
        Status::Done,
        "the receded obs is the completion"
    );
    assert_eq!(
        after.receded[0].1.completed_epoch_s,
        Some(200),
        "original stamp rides along"
    );
}
...
fn error_is_exempt_from_ttl_but_counts_nothing_toward_arming() {
    ...
    assert!(
        !s.has_done_awaiting_recede(),
        "Error must not pin the timer"
    );
}
...
fn promotion_preserves_running_since_for_same_command() {
    ...
    assert_eq!(
        s.get(1).unwrap().last_change_tick,
        t0,
        "same command keeps its start tick"
    );
    assert!(
        !r.changed,
        "identical re-promotion stores an identical observation — no snapshot write"
    );
}
...
fn promotion_over_a_done_reports_the_displaced_completion() {
    ...
    assert_eq!(
        r.receded.len(),
        1,
        "the old Done left the card via overwrite"
    );
...
fn fresh_instance_exit_replay_of_displayed_completion_is_inert() {
    ...
    for (code, status) in [(Some(0), Status::Done), (Some(2), Status::Error)] {
        ...
        assert!(
            receded.is_none(),
            "{status:?}: an identical replay must not ghost-ledger"
        );
        let got = s.get(9).unwrap();
        assert_eq!(got.status, status, "{status:?}: unchanged");
        assert_eq!(
            got.completed_epoch_s,
            Some(100),
            "{status:?}: original stamp survives"
        );
        assert_eq!(
            got.last_change_tick, 5,
            "{status:?}: no tick bump — the TTL clock must keep running"
        );
        ...
        assert!(
            receded.is_none(),
            "{status:?}: dedup primed after the first swallow"
        );
...
fn different_exit_code_on_displayed_completion_still_displaces() {
    ...
    let receded = s
        .on_exit(9, Some(2), Tick(50), EpochSecs(999))
...
fn exit_before_promotion_wears_the_pending_commands_label() {
    ...
    assert_eq!(
        got.repo, "myrepo",
        "repo derived from the pending cwd, like promotion"
    );
...
fn rerun_exit_before_promotion_does_not_wear_the_previous_runs_label() {
    ...
    let receded = s
        .on_exit(1, Some(101), Tick(rerun + 1), EpochSecs(300))
        .expect("the old Done is displaced");
    assert_eq!(receded.status, Status::Done);
    assert_eq!(
        receded.msg, "cargo test",
        "the ledgered completion is the OLD run"
    );
    let got = s.get(1).unwrap();
    assert_eq!(got.status, Status::Error);
    assert_eq!(
        got.msg, "cargo build",
        "the failure wears the NEW run's label"
    );
    assert_eq!(got.kind, Kind::Build);
...
fn rerun_with_command_changed_after_recede_applies_its_exit() {
    ...
    assert_eq!(
        s.get(9).unwrap().status,
        Status::Done,
        "new run's completion applies"
    );
...
fn childless_interactive_root_still_records_quiet_pending() {
    ...
    assert!(
        s.get(1).is_none(),
        "quiet pending never promotes to Running"
    );
...
fn sweep_skips_a_row_already_leaving_the_foreground() {
    ...
    assert_eq!(
        s.quiet_identity(1),
        None,
        "no ghost label for a prompt pane"
    );
    assert_eq!(
        s.get(1).unwrap().status,
        Status::Running,
        "left for the confirm"
    );
...
fn set_interactive_extras_sweeps_promoted_rows_level_triggered() {
    ...
    assert_eq!(
        s.get(2).unwrap().status,
        Status::Running,
        "un-quieted and promoted"
    );
}
...
fn set_remote_extras_composes_defaults_and_extras() {
    ...
    assert_eq!(
        s.get(1).unwrap().kind,
        Kind::Command,
        "removing the extra reverts the kind"
    );
}
...
fn set_remote_extras_is_a_no_op_for_default_remote_names() {
    ...
    assert_eq!(
        s.get(1).unwrap().kind,
        Kind::Command,
        "trailing remote command stays a Job"
    );
...
fn mosh_bootstrap_lands_on_the_client_label() {
    ...
    s.on_command_changed(
        1,
        &argv(&["mosh-client", "1.2.3.4", "60001"]),
        true,
        None,
        1,
    );
    ...
    let obs = s.get(1).unwrap();
    assert_eq!(
        (obs.msg.as_str(), obs.kind),
        ("mosh-client 1.2.3.4", Kind::Remote)
    );
}
...
fn set_remote_extras_leaves_non_remote_kinds_alone() {
    ...
    assert!(
        !s.set_remote_extras([]),
        "empty extras: nothing observable changes"
    );
    assert!(
        !s.set_remote_extras(["distrobox"]),
        "an extra no row uses: still nothing"
    );
    assert_eq!(s.get(1).unwrap().kind, Kind::Server);
    assert_eq!(s.get(2).unwrap().kind, Kind::Test);
    assert_eq!(s.get(3).unwrap().kind, Kind::Build);
    assert!(
        !s.get(1).unwrap().animating(),
        "the dev server stays steady"
    );
    s.on_timer(Tick(2 * DEBOUNCE_TICKS), EpochSecs(100));
    assert_eq!(
        s.get(4).unwrap().kind,
        Kind::Test,
        "a pending's kind survives the sweep"
    );
}
...
fn display_remote_keeps_scanning_options_after_the_destination() {
    ...
    assert_eq!(
        classify(&argv(&["ssh", "prod-db", "-p", "2222"])),
...
fn display_remote_uses_per_tool_option_tables() {
    ...
    assert_eq!(
        classify(&argv(&["autossh", "-M", "0", "prod-db"])),
        ("autossh prod-db".into(), Kind::Remote)
    );
    assert_eq!(
        classify(&argv(&["ssh", "-M", "prod-db"])),
        ("ssh prod-db".into(), Kind::Remote)
    );
    // et's and mosh's separated-value long options are skipped with their
    // value, so the real host is still the destination.
    assert_eq!(
        classify(&argv(&["et", "--jumphost", "bastion", "prod-db"])),
        ("et prod-db".into(), Kind::Remote)
    );
    assert_eq!(
        display(&argv(&["et", "--jport", "2022", "prod-db:2022"])),
        "et prod-db"
    );
    assert_eq!(
        classify(&argv(&["mosh", "--family", "inet", "prod-db"])),
        ("mosh prod-db".into(), Kind::Remote)
    );
    // `--` alone, and `--` before both destination and command.
    assert_eq!(
        classify(&argv(&["ssh", "--"])),
        ("ssh".into(), Kind::Command)
    );
    assert_eq!(
        classify(&argv(&["ssh", "--", "prod-db", "-p"])),
        ("ssh prod-db".into(), Kind::Command)
    );
}
...
fn js_runtime_agent_is_never_opened_as_a_command() {
    ...
    assert!(
        store.get(1).is_none(),
        "a node-hosted agent must stay push-owned"
    );
    ...
    assert!(
        store.get(2).is_some(),
        "a plain node script still opens a lifecycle"
    );
}
