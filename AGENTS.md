...
of the core is host-testable — no wasm build needed for typical work.
...
- `docs/rail-reference.md` is an executable spec — it is `include_str!`'d by
  `src/reference_tests.rs`. Edit it through that test, not casually.

## Adding a producer or agent

The only external interface is the versioned `zj_radar.status.v1` pipe payload.
New instrumented agent → `enum Agent` variant in `src/cli/agents/` +
...
in `src/command.rs`, not in `agents/`.
